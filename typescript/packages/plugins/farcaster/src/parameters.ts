...
export const searchCastsParametersSchema = z
    .object({
        query: z.string().describe("Search query string"),
        author_fid: z.number().optional().describe("Filter results by author FID"),
        viewer_fid: z.number().optional().describe("Viewer FID for personalized results"),
...
export const getConversationSummaryParametersSchema = z.object({
    identifier: z.string().describe("Cast URL or hash identifier"),
    limit: z.number().min(1).max(50).optional().default(20).describe("Number of casts to consider in summary"),
    prompt: z.string().optional().describe("Additional prompt for summary generation"),
});

export const bulkGetCastsParametersSchema = z.object({
    hashes: z.array(z.string()).min(1).max(100).describe("Array of cast hashes to fetch"),
    viewer_fid: z.number().optional().describe("Viewer's FID to include viewer-specific context"),
});

export const deleteCastParametersSchema = z.object({
    signer_uuid: z.string().describe("UUID of the signer who owns the cast"),
    hash: z.string().describe("Hash of the cast to delete"),
});

export const FarcasterEmbedSchema = z
    .object({
        url: z.string().optional(),
        castId: z
            .object({
...
export const PublishCastOptionsSchema = z.object({
    parent: z.string().optional(),
    channelId: z.string().optional(),
    embeds: FarcasterEmbedSchema.optional(),
    parentAuthorFid: z.number().optional(),
    idem: z.string().optional(),
});

export type FarcasterEmbed = z.infer<typeof FarcasterEmbedSchema>;
export type PublishCastOptions = z.infer<typeof PublishCastOptionsSchema>;
